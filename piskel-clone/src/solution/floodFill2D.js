...
const floodFill = (function () {
  ...
  let keepMask = false; // if true then a mask of the filled area is returned as a canvas image
  let extentOnly = false; // if true then the extent of the fill is returned
  let copyPixels = false; // if true then creating a copy of filled pixels
  let cutPixels = false; // if true and copyPixels true then filled pixels are removed
  let useBoundingColor = false; // Set the colour to fill up to. Will not fill over this colour
  let useCompareColor = false; // Rather than get the pixel at posX,posY use the compareColours
  let red;
  let green;
  let blue;
  let alpha; // compare colours if
  let canvas; let ctx;
  function floodFill(posX, posY, tolerance, context2D, diagonal, area, toleranceFade) {
        let w; let h; let painted; let x; let y; let ind; let sr; let sg; let sb; let sa; let imgData; let data; let data32; let RGBA32; let stack; let stackPos; let lookLeft; let lookRight; let i; let colImgDat; let differance; let checkColour;
        ...
        function checkColourAll(ind) {
            ...
            let ind4 = ind << 2; // get index of pixel
        ...
        function checkColourBound(ind) {
            ...
            const ind4 = ind << 2; // get index of pixel
            differance = 0;
    ...
    return {
        fill(posX, posY, tolerance, context2D, diagonal, area, toleranceFade) {
            floodFill(posX, posY, tolerance, context2D, diagonal, area, toleranceFade);
            ctx = undefined;
            canvas = undefined;
        },
        getMask(posX, posY, tolerance, context2D, diagonal, area, toleranceFade) {
            keepMask = true;
            floodFill(posX, posY, tolerance, context2D, diagonal, area, toleranceFade);
            ctx = undefined;
            keepMask = false;
        ...
        getExtent(posX, posY, tolerance, context2D, diagonal, area, toleranceFade) {
            extentOnly = true;
        ...
        cut(posX, posY, tolerance, context2D, diagonal, area, toleranceFade) {
            cutPixels = true;
            copyPixels = true;
            floodFill(posX, posY, tolerance, context2D, diagonal, area, toleranceFade);
            cutPixels = false;
        ...
        copy(posX, posY, tolerance, context2D, diagonal, area, toleranceFade) {
            cutPixels = false;
            copyPixels = true;
            floodFill(posX, posY, tolerance, context2D, diagonal, area, toleranceFade);
            copyPixels = false;
